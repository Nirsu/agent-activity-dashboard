...
type PriceResolver = (model: string, at: number) => ModelPrice | undefined;
let catalogueResolver: PriceResolver | undefined;
export function usePricingCatalogue(resolver: PriceResolver): () => void {
  catalogueResolver = resolver;
  return () => {
    if (catalogueResolver === resolver) {
      catalogueResolver = undefined;
    }
  };
}
...
export function estimateModelCost(
  ...
  if (
    ![usage.inputTokens, usage.outputTokens, cached].every(
  ...
  if (
    cached > 0 &&
    (typeof price.cachedInputPerMillionUsd !== 'number' ||
      !Number.isFinite(price.cachedInputPerMillionUsd) ||
      price.cachedInputPerMillionUsd < 0)
  ) {
    return null;
  }
  const amount =
    ((usage.inputTokens - cached) * price.inputPerMillionUsd +
